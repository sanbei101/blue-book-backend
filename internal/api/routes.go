...
func RegisterRoutesWithMedia(store *db.Store, presigner *media.Presigner) *chi.Mux {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.With(jwt.OptionalAuthMiddleware).Get("/search/suggestions", discoveryHandler.Suggestions)
		r.Get("/search/trending", discoveryHandler.Trending)
		r.With(jwt.OptionalAuthMiddleware).Get("/feed/recommended", discoveryHandler.Recommended)
		r.Get("/tags/{tag_id}", discoveryHandler.GetTag)
		r.With(jwt.OptionalAuthMiddleware).Get("/tags/{tag_id}/posts", discoveryHandler.ListTagPosts)
		r.Get("/topics", discoveryHandler.ListTopics)
		r.Get("/topics/{topic_id}", discoveryHandler.GetTopic)
		r.With(jwt.OptionalAuthMiddleware).Get("/topics/{topic_id}/posts", discoveryHandler.ListTopicPosts)

		// 公开路由
		r.With(jwt.OptionalAuthMiddleware).Get("/users/{user_id}", userHandler.GetProfile)
		r.With(jwt.OptionalAuthMiddleware).Get("/posts", postHandler.ListFeed)
		r.With(jwt.OptionalAuthMiddleware).Get("/posts/{post_id}", postHandler.GetByID)
		r.With(jwt.OptionalAuthMiddleware).Get("/posts/user/{user_id}", postHandler.ListByUser)
		r.With(jwt.OptionalAuthMiddleware).Get("/users/{user_id}/posts", postHandler.ListByUser)
		r.With(jwt.OptionalAuthMiddleware).Get("/posts/{post_id}/comments", commentHandler.ListByPost)
		r.With(jwt.OptionalAuthMiddleware).Get("/users/{user_id}/followers", followHandler.ListFollowers)
		r.With(jwt.OptionalAuthMiddleware).Get("/users/{user_id}/following", followHandler.ListFollowing)
